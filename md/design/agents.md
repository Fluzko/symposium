...
in its hook proto. It fires with the first turn of a new conversation (in the
TUI, at the first prompt rather than at launch) and never on resume, so a
resumed conversation skips the `SessionStart` refresh. There is no prompt event,
so `PreInvocation` stands in for `user-prompt-submit`; because it fires before
every model call, only `invocationNum == 0` dispatches
(`AgentHookInput::should_dispatch`).
