...
//!    prompt event and `PreInvocation` fires before *every* model call, so only
//!    `invocationNum == 0` dispatches (`AgentHookInput::should_dispatch`).
//!
...
impl AgentHookInput for AntigravityInvocationInput {
    ...
    /// `PreInvocation` fires before every model call, several times in a turn
    /// that uses tools; only the first call of a turn is the prompt event.
    fn should_dispatch(&self) -> bool {
        self.invocation_num == 0
    }
    fn into_any(self: Box<Self>) -> Box<dyn std::any::Any> {
        self
    }
...
mod tests {
    ...
    fn only_the_first_invocation_of_a_turn_is_a_prompt() {
        let parse = |s: &str| AntigravityInvocationInput::parse_input(s).unwrap();
        assert!(parse(r#"{"invocationNum":0}"#).should_dispatch());
        assert!(!parse(r#"{"invocationNum":1}"#).should_dispatch());
        assert!(!parse(r#"{"invocationNum":7}"#).should_dispatch());
        // A missing counter is invocation 0, so a wire change fires too often
        // rather than never.
        assert!(parse(r#"{}"#).should_dispatch());
    }
}
